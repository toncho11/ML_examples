...
for subject in dataset.subject_list[0:17]:
...
for subject in dataset.subject_list[17:]:
...
model.compile(optimizer='adam', #This is how the model is updated based on the data it sees and its loss function.
              loss=tf.keras.losses.BinaryCrossentropy(), #This measures how accurate the model is during training. You want to minimize this function to "steer" the model in the right direction.
...
model.fit(train_images, train_labels, batch_size=6, epochs=4, validation_data=(test_images,test_labels))
