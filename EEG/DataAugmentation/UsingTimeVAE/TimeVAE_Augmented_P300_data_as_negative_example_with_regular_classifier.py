...
P300 augmented data (generated from P300 epochs) is used as a bad example of P300.
The idea is to create fake P300 samples and train a classifier to make the difference between the two.
This idea is inspired by GAN.

- real P300 + Interpolated data is used as P300 class.
- real Non P300 + Augmented (from P300) is used as Non P300 class. 
"""
...
def BuidlDataset(datasets, selectedSubjects, restrictEpoch):
...
def Evaluate(X_train, X_test, y_train, y_test):
    ...
    clf = make_pipeline(XdawnCovariances(4),
    TangentSpace(),
...
def TrainVAE(X, y, selected_class, iterations, hidden_layer_low, latent_dim):
    ...
    N, T, D = X_1_class.shape #N = number of samples, T = time steps, D = feature dimensions
    print(N, T, D)
    # X = X.reshape(N,D,T)
    X_1_class = X_1_class.transpose(0,2,1)
    N, T, D = X_1_class.shape
    print(N, T, D)
...
if __name__ == "__main__":
    ...
    #bi2015a        32  43
    #bi2015b        32  44
    #bi2013a gives NaNs in the augmented data
    ds = [BNCI2014009()] #bi2014a() bi2013a()
    iterations = 10
    iterationsVAE = 500 #more means better training for VAE
    selectedSubjects = list(range(1,11))
    restrictEpoch = False
    adjustNonP300Class = True
    # init
    pure_mdm_scores = []
    aug_mdm_scores  = []
    #aug_filtered_vs_all = [] #what portion of the newly generated samples looked like P300 according to MDM
    
    X, y = BuidlDataset(ds, selectedSubjects, restrictEpoch)
    ...
    for i in range(iterations):
        ...
        for hl in [700]:#700, 900, 2000 , default 500
            for ls in [8]: #16 produces NaNs
                ...
                for percentage in [25]:#[2, 3, 10, 15, 20]: #, 5, 10, 20
                    
                    print("% P300 Added:", percentage)
                    
                    samples_required = int(percentage * NonTargetCount / 100) #7000#3000#int(percentageP300Added * P300ClassCount / 100)  #5000 #default 100
                    print("samples_required", samples_required)
                    ...
                    y_train = np.concatenate((y_train, np.repeat(NonP300Class,X_augmented.shape[0])), axis=0)
                    ...
                    #print(CR2)
                    print("[[ iteration: base / augmented", i, ": ", pure_mdm_ba, "/" , ba_augmented, " ]]")
                    print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
...
print("classification original / classification augmented:", np.mean(pure_mdm_scores), "/", np.mean(aug_mdm_scores), "Difference: ",np.mean(np.array(pure_mdm_scores)-np.array(aug_mdm_scores)) * 100, "%")
