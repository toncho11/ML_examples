...
for epoch in range(num_epochs):
    ...
    for n, (real_samples, _) in enumerate(train_loader):
        ...
        real_samples_labels = torch.ones((batch_size, 1)).to(device=device) #real samples are labeled with 1
        latent_space_samples = torch.randn((batch_size, 2)).to(device=device)
        
        generated_samples = generator(latent_space_samples)
        generated_samples_labels = torch.zeros((batch_size, 1)).to(device=device) #generated are labeled with 0
